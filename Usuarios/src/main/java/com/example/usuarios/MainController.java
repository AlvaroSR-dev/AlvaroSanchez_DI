...
public class MainController implements Initializable {
    ...
    @FXML
    private Button botonSuma, botonResta, botonDiv, botonMulti, botonIgual, botonMostrar, botonOcultar, botonComprobar, buttonFiltrar;
    @FXML
    ...
    @FXML
    private TextArea resultadoFiltro;

    @FXML
    private MenuButton seleccionGenero;

    @FXML
    private MenuItem selectFemenino, selectMasculino, selectTodos;
    // ArrayList

    private ObservableList<String> listaCombo, listaChoice, listaSpinner, listaListView;
    private ObservableList<Integer> listaNumeros;
    ...
    private void filtrarJSON(String genero, Integer resultados){

        String urlString = "https://randomuser.me/api/?gender="+genero+"&?results="+resultados;
        try {
            // 1- URL
            URL url = new URL(urlString);
            // 2- Crear la conexion
            ...
            for (int i = 0; i < resultados; i++) {
                ...
                String first = item.getJSONObject("name").getString("first");
                String last = item.getJSONObject("name").getString("last");
                resultadoFiltro.setText(resultadoFiltro.getText() + title + " " + first + " " + last + "\n");
            }
    ...
    private void asociarDatos() {
        ...
        spinnerResultados.setValueFactory(new SpinnerValueFactory.ListSpinnerValueFactory<Integer>(listaNumeros));
    }
    ...
    private void acciones() {
        ...
        for (MenuItem menuItem : seleccionGenero.getItems()) {
            menuItem.setOnAction(new EventHandler<ActionEvent>() {
                @Override
                public void handle(ActionEvent actionEvent) {
                    seleccionGenero.setText(menuItem.getText());
                }
            });
        ...
        buttonFiltrar.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent actionEvent) {
                resultadoFiltro.setText("");
                filtrarJSON(seleccionGenero.getText(), spinnerResultados.getValue());
            }
        });
        }
    }
